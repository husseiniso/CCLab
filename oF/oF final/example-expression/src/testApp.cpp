...
void testApp::draw() {
    ...
    if(tracker.getFound()){
        ...
        if ((NoseSize<40 && MouthStretch>75)||
            //(NoseSize>64 && NoseSize<= 80 && MouthStretch>80)||
            //((NoseSize>40 && NoseSize<= 50 && MouthStretch>57)||
            //((NoseSize>50 && NoseSize<=64 && MouthStretch>60) ||
            (NoseSize>65 && MouthStretch>95))
            
            
           //Color the scene in colored
            ofSetColor(0,255,255);
        
        if ((NoseSize<40 && MouthStretch<55)||
            
            (NoseSize>=40 && MouthStretch<70))
        
            ofSetColor(0,0 ,255);
        
        if
            
            ((NoseSize>=40 && MouthStretch<80))
    ...
    drawHighlightString(
                        string() +
                        "spread your arms to touch the screen"+
                        "r - reset face tracking \n"+
                        "smile for green (horizontal wide mouth) \n" +
                        "serious face for red (tight mouth) \n"+
                        "be amazed for blue (smallest mouth) \n",
                       100, ofGetHeight() - 10 * 12);
